...
home_sum_stats<-stats%>%filter(match_id%in%home_matches$match_id)%>%summarise(home_attigi_GoalAttempts=mean(home_GoalAttempts),
...
away_sum_stats<-stats%>%filter(match_id%in%away_matches$match_id)%>%summarise(away_attigi_GoalAttempts=mean(away_GoalAttempts),
