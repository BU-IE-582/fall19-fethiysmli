...
ggplot(pl,aes(x=match_hometeam_score))+geom_histogram()+ylab("Number of Games")+xlab("home_score")+geom_line(aes(y=yfit_away),color="blue")


###### Task 2
pl_bets<-subset(bets1,bets1$match_id%in%pl$match_id) ## only PL games
pl_betters<-unique(pl_bets$odd_bookmakers) 
selected_bookmakers<-pl_betters[1:4] ## select 4 gook makers
...
## Task 2 Part 3
for(i in 1:4){  
  set1<-subset(pl_bets,pl_bets$odd_bookmakers==pl_betters[i])
  set1<-set1%>%arrange(match_id,variable)
  set1<-set1%>%group_by(match_id)%>%mutate(predictor=first(classical_prob)-nth(classical_prob,2))## calculates p(home win)-p(away win)
  draws_probs<-subset(set1,set1$variable=="odd_x") ## only draws
...
count=length(setdiff(pl_bets$match_id,remaining_bets$match_id))
for(i in 1:4){
set3<-subset(remaining_bets,pl_bets$odd_bookmakers==pl_betters[i])
  ...
  draws_probs_new2<-merge(draws_probs_new2,bin_probs_new2,by="bin")
   p<-ggplot(draws_probs_new2,aes(x=predictor,shape=draws_probs_new2$bin))+geom_point(aes(y=classical_prob))+geom_point(aes(y=actual_prob,),col="blue")+ scale_shape_identity() +xlab("P(home win) – P(away win win)")+ylab("P(1/odd)")+ggtitle(paste(pl_betters[i],"-",count,"match romoved",sep=" "))
  plot(p)
}
